...
class BPredUnit : public SimObject
{
  public:
     ...
     */
    bool predict(StaticInstPtr &inst, const InstSeqNum &seqNum,TheISA::PCState &pc, ThreadID tid);
    bool predictInOrder(StaticInstPtr &inst, const InstSeqNum &seqNum,
                        int asid, TheISA::PCState &instPC, TheISA::PCState &predPC,
                        ThreadID tid);

    // @todo: Rename this function.
    virtual void uncondBranch(void * &bp_history, Addr instPC ) = 0;
    ...
    virtual void squash2(Addr &branch_addr)=0; 
    /**
  ...
  private:
    ...
    /** no of times predictor counters are updated*/
   Stats::Scalar UpdatePredictor;
   /* predictor hit percentage*/
   Stats::Formula  correctpredictionPercent;

   Stats::Formula correct_pred;

    // added
   char msg1[100];
   file fl1;
  // std::vector<statistics> report;
  unsigned lookup_count_in;
  unsigned lookup_count;
  unsigned mispredict_count;
  unsigned predicted_count;
  unsigned predicted_count_in;
  unsigned BTB_count_in;
  unsigned BTBhit_count_in;
  unsigned BTB_count;
  unsigned BTBhit_count;
